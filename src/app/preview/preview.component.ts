...
import { faVolumeUp, faVolumeMute, faCodeBranch, faGamepad } from '@fortawesome/free-solid-svg-icons';
...
export class PreviewComponent implements AfterViewInit {
  ...
  scene: any = null;
  juliaMesh: any = null;
  juliaRenderer: any = null;
  juliaCamera: any = null;
  juliaScene: any = null;
  scrollSpeed: number = 0;
  transitionSpeed: number = 0;
  fractalX: number = -.5;
  fractalY: number = 0;
  futureFractalX: number = this.fractalX;
  futureFractalY: number = this.fractalY;
  zoom: number = 1;
  width: number = 0;
  height: number = 0;
  ...
  audioContext: any;
  biquadFilter: any;
  juliaWidth: number = 300;
  juliaHeight: number = 200;
  ...
  ngAfterViewInit() {
    ...
    // Print Version
    console.log("%cFrac v" + version, "background-color: #1e90ff; color: #fff; font-size: 16px; padding: 8px 16px; border-radius: 100px;")

    // Load audio
    let prodPath = `../../Frac/assets/sounds/ambient.wav`;
    let devPath = `../../assets/sounds/ambient.wav`;
    this.audio = new Audio(isDevMode() ? devPath : prodPath);
    this.audio.loop = true;
    this.audio.autoplay = true;

    // Create a biquadfilter to emphasize bass while scrolling
  ...
  async init() {
    ...
    // Get preview element and its width and height
    ...
    this.audio.volume = this.getCookie("muted") !== "false"? this.MUSIC_VOLUME : 0;
  ...
  render() {
    ...
    // Render scene
    this.loopHandler = requestAnimationFrame(() => this.render());
  ...
  handleZoomScrolling() {
    ...
    this.biquadFilter.gain.value = Math.abs(this.MUSIC_SCROLL_SENSITIVITY * this.scrollSpeed);

    // Stop the scroll if it becomes unrecognizably slow
    if (Math.abs(this.scrollSpeed) < 0.01)
      this.scrollSpeed = 0;
  }
  ...
  handleOffsetScrolling() {
    ...
    const deltaX = futureOffsetX / (this.TRANSITION_SLOWNESS / Math.abs(this.scrollSpeed));
    const deltaY = futureOffsetY / (this.TRANSITION_SLOWNESS / Math.abs(this.scrollSpeed));

    this.fractalX += deltaX;
    this.fractalY += deltaY;
    this.updateUniforms(this.mesh, { offsetX: this.fractalX, offsetY: this.fractalY });
  }
   ...
   */
  updateUniforms(mesh: any, uniforms?: any) {
    if (!mesh) return;

    // Apply offset changes
    if (uniforms?.offsetX && uniforms?.offsetY && !this.juliaMode) {
      mesh.material.uniforms.u_offset = new THREE.Uniform(new THREE.Vector2(uniforms.offsetX, uniforms.offsetY));
    }
    ...
    if (uniforms?.cursorPositionX && uniforms?.cursorPositionY) {
      mesh.material.uniforms.u_cursor_position = new THREE.Uniform(new THREE.Vector2(this.cursorPositionX, this.cursorPositionY));
    }
  ...
  async toggleJuliaView() {
    ...
    this.mesh = new THREE.Mesh(new THREE.PlaneGeometry(), this.juliaMode ? jmaterial : material);
    this.scene.add(this.mesh);

    // Update Julia scene
    this.juliaScene = new THREE.Scene();
    this.juliaMesh = new THREE.Mesh(new THREE.PlaneGeometry(), this.juliaMode ? material : jmaterial);
  ...
  */
  async createTHREEMandelbrotEnviroment() {
    this.scene = new THREE.Scene();
    this.camera = new THREE.OrthographicCamera(-.5, .5, .5, -.5, -1, 1);
  ...
  async createMandelbrotMaterial() {
    return new THREE.ShaderMaterial({
      uniforms: {
        ...
        u_offset: new THREE.Uniform(new THREE.Vector2(this.fractalX, this.fractalY)),
  ...
  */
  async createTHREEJuliaEnviroment() {
    this.juliaScene = new THREE.Scene();
    this.juliaCamera = new THREE.OrthographicCamera(-.5, .5, .5, -.5, -1, 1);
    ...
    this.juliaRenderer.domElement.addEventListener("click", () => this.toggleJuliaView.call(this));
  ...
  async createJuliaMaterial() {
    return new THREE.ShaderMaterial({
      uniforms: {
        ...
        u_colormode: { value: this.colorMode },
        u_modifiermode: { value: this.modifierMode },
        u_cursor_position: new THREE.Uniform(new THREE.Vector2(this.cursorPositionX, this.cursorPositionY)),
      },
  ...
  getGPU() {
    ...
    const renderer = gl?.getParameter(debugInfo?.UNMASKED_RENDERER_WEBGL ?? -1);
    ...
    const directInfoIndex = card?.indexOf("Direct") ?? -1;
    return directInfoIndex > 0? card?.substring(0, directInfoIndex).trim() : card;
  }
  ...
  zoomIn(event: any) {
    ...
    const newFractalX = this.fractalX + this.TRANSITION_SENSITIVITY * ratioX * fractalWidth * this.width / this.height;
    const newFractalY = this.fractalY + this.TRANSITION_SENSITIVITY * ratioY * fractalWidth;
  ...
  mouseMove(event: any) {
    ...
    this.cursorPositionX = (this.dragMode? this.futureFractalX : this.fractalX) + ((event.x / this.width) - 0.5) * fractalWidthX;
    this.cursorPositionY = (this.dragMode? this.futureFractalY : this.fractalY) - ((event.y / this.height) - 0.5) * fractalWidthY;
    ...
    const newFractalX = this.fractalX + (fractalWidthX * deltaPixelsX) / this.width;
    const newFractalY = this.fractalY + (fractalWidthY * deltaPixelsY) / this.height;

    this.futureFractalX = newFractalX;
    this.futureFractalY = newFractalY;
    this.updateUniforms(this.mesh, { offsetX: newFractalX, offsetY: newFractalY });
  }
  ...
  increaseIterations(event: any) {
    ...
    this.updateUniforms(this.juliaMesh, { iterations: Math.floor(this.iterations) });
  }
  ...
  saveDialog(event: any) {
    let saveDialog = document.getElementById("save");
    if (!saveDialog)
      return;
  ...
  download(filename: string, dataUrl: any) {
  ...
  toggleHUD() {
    ...
    const juliaElement = juliaElements.length? juliaElements[0] as HTMLElement : null;
